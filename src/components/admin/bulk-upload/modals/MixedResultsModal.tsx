...
import { CheckCircle, XCircle, X, RefreshCw, ExternalLink } from 'lucide-react';
...
export default function MixedResultsModal({ results, onClose, onRetry }: MixedResultsModalProps) {
  const totalAttempted = results.successful.length + results.failed.length;
  const successRate = Math.round((results.successful.length / totalAttempted) * 100);
      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-semibold mb-2">Upload Partially Complete</h2>
              <p className="text-gray-600">
                {results.successful.length} of {totalAttempted} products uploaded successfully ({successRate}% success rate)
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-green-600 mb-1">{results.successful.length}</div>
              <p className="text-sm text-green-700">Products added to gallery</p>
            </div>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-red-600 mb-1">{results.failed.length}</div>
              <p className="text-sm text-red-700">Products need attention</p>
            </div>
          </div>
